    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-[#FFEBFA] rounded-2xl shadow-lg p-6 mb-6">
            <h1 class="text-3xl font-extrabold text-[#3A4454] flex items-center gap-2">
                @svg('heroicon-o-plus-circle', 'w-8 h-8 text-[#6B4E71]')
                Create New Review
            </h1>
        </div>
            ...
            <form action="{{ route('admin.reviews.store') }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- User Selection -->
                        <div>
                            <label for="user_id" class="block text-lg font-semibold text-[#3A4454] mb-2">
                                @svg('heroicon-o-user', 'w-5 h-5 inline mr-1 text-[#6B4E71]') User
                            </label>
                            <select name="user_id" id="user_id" required
                                class="w-full px-4 py-3 bg-[#FFEBFA] rounded-xl border border-[#D7C1D3] focus:ring-[#6B4E71] focus:border-[#6B4E71]">
                        ...
                        <!-- Event Selection -->
                        <div>
                            <label for="event_id" class="block text-lg font-semibold text-[#3A4454] mb-2">
                    ...
                    <!-- Rating -->
                    <div>
                        <label class="block text-lg font-semibold text-[#3A4454] mb-3">
                    ...
                    <div>
                        ...
                        <textarea name="comment" id="comment" rows="5"
                            class="w-full px-4 py-3 bg-[#FFEBFA] rounded-xl border border-[#D7C1D3] focus:ring-[#6B4E71] focus:border-[#6B4E71]">{{ old('comment') }}</textarea>
                        @error('comment')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
