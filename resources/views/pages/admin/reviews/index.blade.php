    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-[#FFEBFA] rounded-2xl shadow-lg p-6 mb-6">
            <form method="GET" class="flex flex-col md:flex-row gap-4">
                <input
                    ...
                    class="px-4 py-2 rounded-lg border border-[#D7C1D3] focus:ring-[#6B4E71] focus:border-[#6B4E71]"
                >
                <select
                    name="rating"
                    class="px-4 py-2 rounded-lg border border-[#D7C1D3] focus:ring-[#6B4E71] focus:border-[#6B4E71]"
                >
                    ...
                    @foreach($ratings as $rating)
                        <option value="{{ $rating }}" {{ request('rating') == $rating ? 'selected' : '' }}>
                            {{ $rating }} @if($rating == 1) star @else stars @endif
                        </option>
                    @endforeach
                </select>
                <select
                ...
                <button
                    type="submit"
                    class="px-4 py-2 bg-[#6B4E71] text-white rounded-lg hover:bg-[#593b5c] transition flex items-center gap-2"
                >
                    @svg('heroicon-o-funnel', 'h-5 w-5')
                    Filter
                </button>
                <a
                    href="{{ route('admin.reviews.index') }}"
                    class="px-4 py-2 border border-[#6B4E71] text-[#6B4E71] rounded-lg hover:bg-[#6B4E71] hover:text-white transition flex items-center gap-2"
                >
        ...
        <div class="bg-[#FFEBFA] rounded-2xl shadow-lg p-6 mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm border border-[#D7C1D3]">
                ...
                <div class="text-2xl font-bold text-[#3A4454] mt-2">{{ $lastReviewDate?->format('d.m.Y') ?? 'None' }}</div>
            </div>
        </div>
            ...
            @else
                <div class="space-y-4">
                    @foreach ($reviews as $review)
                        <div class="bg-white p-5 rounded-xl shadow-sm border border-[#D7C1D3] hover:shadow-md transition">
                            <div class="flex justify-between items-start">
                                ...
                                <div class="flex gap-2">
                                    <a
                                        ...
                                        class="p-2 text-[#6B4E71] hover:bg-[#FFEBFA] rounded-full"
                                        title="Edit"
                                    >
