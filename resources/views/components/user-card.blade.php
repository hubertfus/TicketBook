<div class="bg-[#FFEBFA] rounded-2xl p-6 shadow-lg w-full max-w-xs flex flex-col gap-4 transition-all hover:shadow-md hover:-translate-y-1">
    {{-- User Info --}}
    <div class="flex items-start gap-4">
        <div class="h-12 w-12 flex-shrink-0 flex items-center justify-center rounded-full bg-white border-2 border-[#D7C1D3]">
        ...
        <div class="flex-1 min-w-0">
            <h3 class="text-lg font-bold text-[#3A4454] truncate flex items-center gap-2">
                {{ $user->name }}
                @if($user->role === 'admin')
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[#6B4E71] text-white">
                        Admin
                    </span>
                @endif
            </h3>
